...
class Kernel:
    ...
    def run(this):
        this.result = np.sqrt(
              np.sum(np.square(this.points1) * this.alpha[np.newaxis,:], 1)[:,np.newaxis]
            + np.sum(np.square(this.points2) * this.alpha[np.newaxis,:], 1)[np.newaxis,:]
            - 2 * (this.points1 * this.alpha[np.newaxis,:] @ np.transpose(this.points2))
        )
